...
public sealed class DeviceServiceTests
{
    ...
    public void Create_NormalizesStatusAndAddsDevice()
    {
        ...
        var result = service.Create(new CreateDevice("ECG Monitor","Diagnostic","online","Imaging Lab"));
    ...
    {
        var service = CreateService();
        var result = service.Create(new CreateDevice("ECG Monitor","Diagnostic","Unknown","Imaging Lab"));
    ...
    {
        var service = CreateService();
        var result = service.Create(new CreateDevice("","Diagnostic","Online","Imaging Lab"));
        Assert.NotNull(result.Error);
        Assert.Null(result.Device);
    }
    ...
    {
        var service = CreateService();
        var result = service.Create(new CreateDevice(new string('x', 201),"Diagnostic","Online","Imaging Lab"));
        Assert.NotNull(result.Error);
        Assert.Null(result.Device);
    }
    ...
    {
        var service = CreateService();
        var devices = service.GetDevices("Online","Vital Signs","Home Care");
    ...
    private static DeviceService CreateService() => new(new InMemoryDeviceRepository());
}
